  ...
  return records
    ...
    .map(r => {
      ...
      const doc: KnowledgeDoc = {
        ...
        // 纯文本家族（txt/md/csv）与其它类型同级：缺失/未知类型一律回落 pdf，避免脏数据导致渲染崩溃
        type: (['word', 'ppt', 'excel', 'pdf', 'xml', 'txt', 'md', 'csv'].includes(raw.type as string) ? raw.type : 'pdf') as KnowledgeDoc['type'],
