...
面向制造工厂（消费类聚合物锂离子电池产线）的**局域网私有化 AI 问答系统**。
...
- 超级管理员：用户名 `SITE_ADMIN`（初始密码见 `src/services/userService.ts` 中的 `SUPER_ADMIN`，可用构建期变量 `VITE_INITIAL_ADMIN_PASSWORD` 覆盖）
- **首次登录强制修改密码**，上线后请立即在「用户管理」中重置
...
1. **数据主权**：默认本地模型 + 内网部署，业务数据与代码分离，敏感内容不进仓库、不出内网。
