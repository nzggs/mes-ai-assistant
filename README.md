...
把 MES 业务知识库——现场文档（Word / Excel / PPT / PDF / 纯文本）与从数据库导出的**页面 / 逻辑 / 流程代码（XML）**——统一入库，
...
| **多格式解析** | Word（docx）、Excel（xlsx）、PPT（pptx）、PDF（pdfjs-dist，支持扫描页）、**纯文本（txt，UTF-8/GBK 编码自适应）**、**XML 数据导出** |
...
| **在线预览** | Office 文档浏览器内渲染（docx-preview / SheetJS），PDF 内嵌阅读器，纯文本（txt）等宽原文直出 |
...
**2.1 纯文本文件（txt）**
- 支持直接上传 `.txt`（日志、工艺说明、老系统导出文本等），与其它格式**同等对待**：审核入库、参与检索、可做整篇 AI 总结、可阅读原文与下载原文件。
- **编码自适应**：解码顺序为 BOM（UTF-8 / UTF-16LE / UTF-16BE）→ 严格 UTF-8 → GBK 回退。现场 GBK/GB2312 文本若被按 UTF-8 硬解会整篇乱码，并进一步污染检索与总结，因此必须显式回退。
- **分页**：按行切分为 ≤4000 字的「第N段」，优先在空行（自然段边界）收口、绝不把一行劈开；超长单行（如整条 SQL）按字符硬切。分页粒度同时服务于阅读器翻页、按页检索与总结切片。
