...
无需重启服务。系统**不内置仿真/演示数据源，也不预置监测项目**：未创建项目、项目未配取数
SQL 模板、或绑定的数据库未配置连接时，页面按「未配置数据源」展示空态引导（不展示任何数据）。
...
# APC/RTO 功能与数据源状态（mode=hana 表示项目已就绪、正在读真实 HANA；unconfigured 表示未配置数据源）
...
> 删除该文件等价于「全部恢复为环境变量/默认值」：数据库连接回落到环境变量、监测项目与参数
> 一并清空（页面显示「未配置数据源」空态），系统不会自动生成任何演示项目或仿真数据。
